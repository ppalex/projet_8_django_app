...
class SubstituteViewTest(TestCase):
    ...
    def test_view_url_accessible_by_name(self):
        response = self.client.get("%s?product=product_name_1" % reverse('substitute'))
    ...
    def test_pagination(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEquals(response.context['substitute_pages'].number, response.context['substitute_pages'].paginator.page(1).number)
